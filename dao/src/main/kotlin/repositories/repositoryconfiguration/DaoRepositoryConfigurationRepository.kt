...
class DaoRepositoryConfigurationRepository(private val db: Database) : RepositoryConfigurationRepository {
    override fun create(
        ...
        val vulnerabilityResolutions = mapAndDeduplicate(resolutions.vulnerabilities, VulnerabilityResolutionDao::getOrPut)

        val idToDbVulnerabilityResolutionDaos = VulnerabilityResolutionDefinitionsTable.getMapForRepositoryId(ortRun.repositoryId).mapValues { entry ->
            entry.value.map { vulnerabilityResolution ->
                VulnerabilityResolutionDao.getOrPut(vulnerabilityResolution)
            }
        }

        val combinedVulnerabilityResolutions: SizedIterable<VulnerabilityResolutionDao> = SizedCollection(buildList {
