...
class DaoAnalyzerRunRepository(private val db: Database) : AnalyzerRunRepository {
    override fun create(
        ...
        shortestDependencyPaths: Map<Identifier, ShortestDependencyPath>
    ): AnalyzerRun {
        return db.blockingQuery {
            ...
            val projectsMap = mutableMapOf<Project, ProjectDao>()

            projects.forEach { projectsMap[it] = createProject(analyzerRun, it) }

            packages.forEach {
                val pkgDao = createPackage(analyzerRun, it)

                shortestDependencyPaths[it.identifier]?.also { shortestDependencyPath ->
                    projectsMap[shortestDependencyPath.project]?.also { projectDao ->
