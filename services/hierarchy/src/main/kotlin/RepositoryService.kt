...
class RepositoryService(
        ...
        AnalyzerJobsTable
            ...
            .where {
                (OrtRunsTable.repositoryId eq repositoryId) and
                        (AnalyzerJobsTable.status inList finalJobStatuses())
            }
    ...
    suspend fun getLatestOrtRunIdWithSuccessfulAnalyzerJob(repositoryId: Long): Long? = db.dbQuery {
        AnalyzerJobsTable
            ...
            .where {
                (OrtRunsTable.repositoryId eq repositoryId) and
                        (AnalyzerJobsTable.status inList successfulJobStatuses())
            }
            .orderBy(OrtRunsTable.index, SortOrder.DESC)
            .limit(1)
            .firstOrNull()
    ...
    suspend fun getLatestOrtRunIdWithSuccessfulAdvisorJob(repositoryId: Long): Long? = db.dbQuery {
        AdvisorJobsTable
            ...
            .where {
                (OrtRunsTable.repositoryId eq repositoryId) and
                        (AdvisorJobsTable.status inList successfulJobStatuses())
            }
            .orderBy(OrtRunsTable.index, SortOrder.DESC)
            .limit(1)
            .firstOrNull()
    ...
    suspend fun getLatestOrtRunIdWithSuccessfulEvaluatorJob(repositoryId: Long): Long? = db.dbQuery {
        EvaluatorJobsTable
            ...
            .where {
                (OrtRunsTable.repositoryId eq repositoryId) and
                        (EvaluatorJobsTable.status inList successfulJobStatuses())
            }
            .orderBy(OrtRunsTable.index, SortOrder.DESC)
            .limit(1)
            .firstOrNull()
...
private fun successfulJobStatuses() = listOf(JobStatus.FINISHED, JobStatus.FINISHED_WITH_ISSUES)

private fun finalJobStatuses() = listOf(JobStatus.FINISHED, JobStatus.FINISHED_WITH_ISSUES, JobStatus.FAILED)