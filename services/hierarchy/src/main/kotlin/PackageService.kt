...
import org.eclipse.apoapsis.ortserver.dao.utils.applyLike
...
class PackageService(private val db: Database) {
    suspend fun listForOrtRunId(
        ...
        filters.purl?.let {
            condition = condition and PackagesTable.purl.applyLike(it.value)
        }
        ...
        filters.identifier?.let {
            ...
            condition = condition and (concat1.applyILike(it.value) or concat2.applyILike(it.value))
            //condition = condition and concat2.applyLike(it.value)
        }
